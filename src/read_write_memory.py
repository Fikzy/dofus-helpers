...
class Process(object):
    ...
    def __init__(
        ...
        pid: int = -1,
        handle: int = -1,
        error_code: Union[str, bytes] = None,
    ):
        """
    ...
    def open(self):
        ...
        self.handle = ctypes.windll.kernel32.OpenProcess(
            dw_desired_access, b_inherit_handle, self.pid
        )
        if not self.handle:
    ...
    def get_all_access_handle(self):
        ...
        b_inherit_handle = True
        self.handle = ctypes.windll.kernel32.OpenProcess(
            PROCESS_ALL_ACCESS, b_inherit_handle, self.pid
        )
    ...
    def read(self, lp_base_address: int) -> Any:
        ...
        try:
            ...
            ctypes.windll.kernel32.ReadProcessMemory(
                self.handle,
                ctypes.c_void_p(lp_base_address),
                lp_buffer,
                n_size,
    ...
    def readByte(self, lp_base_address: int, length: int = 1) -> List[hex]:
        ...
        try:
            ...
            for x in range(length):
                ctypes.windll.kernel32.ReadProcessMemory(
                    self.handle,
                    ctypes.c_void_p(lp_base_address + x),
                    lp_buffer,
                    n_size,
                    lp_number_of_bytes_read,
                )
        ...
        if self.process.handle:
            ...
            if (
                ...
                raise ReadWriteMemoryError(
                    f"Unable to get the executable's name for PID={self.process.pid}!"
                )
